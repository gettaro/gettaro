...
type LinesOfCodeAcceptedRule struct {
	metrictypes.BaseMetricRule
	aicodeassistantDB database.DB
}

func NewLinesOfCodeAcceptedRule(baseMetricRule metrictypes.BaseMetricRule, aicodeassistantDB database.DB) *LinesOfCodeAcceptedRule {
	return &LinesOfCodeAcceptedRule{
		BaseMetricRule:     baseMetricRule,
		aicodeassistantDB: aicodeassistantDB,
	}
}

func (r *LinesOfCodeAcceptedRule) Calculate(ctx context.Context, params types.MetricRuleParams) (*types.SnapshotMetric, *types.GraphMetric, error) {
	// Validate params
	...
	// Calculate lines of code accepted value
	locAcceptedValue, err := r.aicodeassistantDB.CalculateLinesOfCodeAccepted(ctx, *organizationID, externalAccountIDs, toolNames, *startDate, *endDate, r.Operation)
	if err != nil {
		return nil, nil, err
	}
	...
	// Calculate lines of code accepted graph value
	locAcceptedGraphValue, err := r.aicodeassistantDB.CalculateLinesOfCodeAcceptedGraph(ctx, *organizationID, externalAccountIDs, toolNames, *startDate, *endDate, r.Operation, r.Name, params.Interval)
	if err != nil {
		return nil, nil, err
	}
...
func (r *LinesOfCodeAcceptedRule) mergeTimeSeriesData(mainSeries []types.TimeSeriesEntry, peersSeries []types.TimeSeriesEntry) []types.TimeSeriesEntry {
...
func (r *LinesOfCodeAcceptedRule) Category() types.MetricRuleCategory {
	return r.BaseMetricRule.Category
}

func (r *LinesOfCodeAcceptedRule) extractParams(params types.MetricRuleParams) (*string, *time.Time, *time.Time, []string, []string, []string, error) {
	if params.Interval == "" {
