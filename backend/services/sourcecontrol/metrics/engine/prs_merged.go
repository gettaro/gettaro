...
func (r *PRsMergedRule) Calculate(ctx context.Context, params types.MetricRuleParams) (*types.SnapshotMetric, *types.GraphMetric, error) {
	...
	prsMergedValue, err := r.sourceControlDB.CalculatePRsMerged(ctx, *organizationID, sourceControlAccountIDs, *startDate, *endDate, r.Operation)
	if err != nil {
		return nil, nil, err
	}

	// Calculate PRs merged peers value
	peersPRsMergedValue, err := r.sourceControlDB.CalculatePRsMergedForAccounts(ctx, *organizationID, peersSourceControlAccountIDs, *startDate, *endDate)
	if err != nil {
		return nil, nil, err
	}
	...
	prsMergedGraphValue, err := r.sourceControlDB.CalculatePRsMergedGraph(ctx, *organizationID, sourceControlAccountIDs, *startDate, *endDate, r.Operation, r.Name, params.Interval)
	if err != nil {
		return nil, nil, err
	}
...
func (r *PRsMergedRule) Category() string {
	return r.BaseMetricRule.Category
}

func (r *PRsMergedRule) extractParams(params types.MetricRuleParams) (*string, *time.Time, *time.Time, []string, []string, error) {
	if params.Interval == "" {
