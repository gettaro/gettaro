...
func NewTimeToMergeRule(baseMetricRule metrictypes.BaseMetricRule, sourceControlDB database.DB) *TimeToMergeRule {
...
func (r *TimeToMergeRule) Calculate(ctx context.Context, params types.MetricRuleParams) (*types.SnapshotMetric, *types.GraphMetric, error) {
	...
	peersTimeToMergeValue, err := r.sourceControlDB.CalculateTimeToMerge(ctx, *organizationID, peersSourceControlAccountIDs, *startDate, *endDate, r.Operation)
	if err != nil {
		return nil, nil, err
	}
...
func (r *TimeToMergeRule) Category() string {
